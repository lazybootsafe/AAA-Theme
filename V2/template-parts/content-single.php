<article class="post post-full card bg-white shadow-sm border-0" id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
	<header class="post-header text-center<?php if (AAA_has_post_thumbnail() && get_option('AAA_show_thumbnail_in_banner_in_content_page') != 'true'){echo " post-header-with-thumbnail";}?>">
		...
		<div class="post-meta">
			...
			?>
			<?php if (!post_password_required() && get_option("AAA_show_readingtime") != "false" && is_readingtime_meta_hidden() == False) { ?>
				</br>
				<div class="post-meta-detail post-meta-detail-words">
					<i class="fa fa-file-word-o" aria-hidden="true"></i>
					<?php
						echo get_article_words_total(get_the_content()) . " " . __("字", 'AAA');
					?>
				</div>
		...
		<?php if (post_password_required()){ ?>
			<div class="text-center container">
				<form action="/wp-login.php?action=postpass" class="post-password-form" method="post">
					<div class="post-password-form-text"><?php _e('这是一篇受密码保护的文章，您需要提供访问密码', 'AAA');?></div>
					<div class="row">
						<div class="form-group col-lg-6 col-md-8 col-sm-10 col-xs-12 post-password-form-input">
							<div class="input-group input-group-alternative">
								...
								<input name="post_password" class="form-control" placeholder="<?php _e('密码', 'AAA');?>" type="password" value="<?php if (current_user_can('level_7')){global $post;if (isset($post -> post_password)){echo esc_attr($post->post_password);}} ?>">
							</div>
							<?php
					...
					<input class="btn btn-primary" type="submit" name="Submit" value="<?php _e('确认', 'AAA');?>">
				</form>
			</div>
		<?php
			}else{
				echo AAA_get_post_outdated_info();
	...
	<?php if (get_option("AAA_donate_qrcode_url") != '') { ?>
		<div class="post-donate">
			<button class="btn donate-btn btn-danger"><?php _e('赞赏', 'AAA');?></button>
			<div class="donate-qrcode card shadow-sm bg-white">
				<img src="<?php echo get_option("AAA_donate_qrcode_url"); ?>">
			</div>
		</div>
	<?php } ?>
