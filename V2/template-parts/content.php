<article class="post card bg-white shadow-sm border-0 <?php if (get_option('AAA_enable_into_article_animation') == 'true'){echo 'post-preview';} ?>" id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
	<header class="post-header text-center<?php if (AAA_has_post_thumbnail()){echo " post-header-with-thumbnail";}?>">
		<?php
			if (AAA_has_post_thumbnail()){
		...
		<div class="post-meta">
			...
			<?php if (!post_password_required() && get_option("AAA_show_readingtime") != "false" && is_readingtime_meta_hidden() == False) { ?>
				...
				<div class="post-meta-detail post-meta-detail-words">
					...
					<?php
						echo get_article_words_total($post_content_full) . " " . __("字", 'AAA');
					?>
				</div>
	...
	<div class="post-content">
		<?php
			if (get_option("AAA_hide_shortcode_in_preview") == 'true'){
				$preview = wp_trim_words(do_shortcode(get_the_content('...')), 175);
			}else{
				$preview = wp_trim_words(get_the_content('...'), 175);
			}
			if (post_password_required()){
				$preview = __("这篇文章受密码保护，输入密码才能阅读", 'AAA');
			}
			if ($preview == ""){
				$preview = __("这篇文章没有摘要", 'AAA');
			}
